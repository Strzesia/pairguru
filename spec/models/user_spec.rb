...
RSpec.describe User, type: :model do
  ...
  describe 'recent_comments_count' do
    let(:user) { create(:user) }
    let!(:recent_comment) { create(:comment, user: user, movie: create(:movie)) }
    let!(:old_comment) { create(:comment, user: user, movie: create(:movie), created_at: Time.current - 8.days) }
